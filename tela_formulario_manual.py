...
def exibir_tela_formulario_manual(base, webhook_url, supervisor, sub):
        ...
        if consulta_concorrentes.get("sucesso"):
            ...
            for nome_base, titulos in consulta_concorrentes.get("dados", {}).items():
                titulos_normalizados = {
                    somente_numeros(titulo_base).lstrip("0")
                    for titulo_base in (titulos or [])
                    if somente_numeros(titulo_base)
                }

                if (
                    titulo_normalizado
                    and titulo_normalizado in titulos_normalizados
                ):
    ...
    if st.session_state.busca_realizada:
        ...
        if bases_manual:
            st.warning(
                "🎯 Cruzamento encontrado: " + " | ".join(bases_manual)
            )
        ...
        if st.session_state.encontrado:
            ...
            st.error(
                f"⚠️ Já cadastrado: {e.get('nome')} | "
                f"Supervisor: {e.get('supervisor')}"
            )
