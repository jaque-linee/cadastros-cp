...
def executar_ocr_imagem(imagem):
    """
    RapidOCR preservando a ORDEM ORIGINAL e os mesmos dados espaciais
    usados pelo extrator testado no VSCode.
    """
    ...
    for i, texto_bruto in enumerate(textos):
        ...
        try:
            confianca = float(scores[i]) if i < len(scores) else 0.0
        except Exception:
            confianca = 0.0
    ...
    # NÃO ordenar: o extrator do VSCode usa a ordem retornada pelo RapidOCR.
...
def extrair_dados_ocr(
    ...
    """
    Usa exatamente o extrator testado no VSCode, mas devolve
    as mesmas chaves minúsculas que o Streamlit já esperava.
    """
    from motor_vscode_rapidocr import extrair_dados as extrair_vscode

    dados = extrair_vscode(itens)
