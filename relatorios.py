...
def obter_filtros_pagamentos_resumidos(
    ...
    """
    Monta os filtros pela união de PAGAMENTOS LIDERANÇAS
    e LIDERANÇAS CONTROLE, para não esconder quem exista
    somente em uma das duas fontes.
    """
    supervisores = set()
    subsupervisores = set()
    comunidades = set()

    fontes = list(dados_pagamentos or []) + list(
        dados_liderancas_controle or []
    )
...
def _chave_lideranca_pagamentos(
    supervisor,
    subsupervisor,
    comunidade
):
    """
    Mantém a mesma identificação usada pelo controle:
    quando existe Subsupervisor, ele identifica a liderança;
    caso contrário, usa Supervisor. Comunidade completa a chave.
    """
    sup = limpar_texto(supervisor)
    sub = limpar_texto(subsupervisor)
    com = limpar_texto(comunidade)

    if sub:
        return (
            "SUB",
            normalizar_filtro(sub),
            normalizar_filtro(com),
        )
...
def _nome_lideranca_pagamentos(supervisor, subsupervisor):
    sub = limpar_texto(subsupervisor)
    sup = limpar_texto(supervisor)
...
def gerar_relatorio_pagamentos_resumidos(
    ...
    **kwargs
    ...
    """
    Relatório resumido para conferência.

    Colunas:
    LIDERANÇA | QTDE | ATUAL | DIFERENÇA |
    TOTAL PAGO | A PAGAR | TOTAL

    DIFERENÇA segue o controle:
    QTDE - ATUAL.
    Ex.: QTDE 30 e ATUAL 37 = -7.

    Uma liderança que exista no controle e não exista em pagamentos
    também aparece, com QTDE e valores financeiros zerados.
    """
    fs = normalizar_filtro(supervisor)
    fsub = normalizar_filtro(subsupervisor)
    fc = normalizar_filtro(comunidade)
    ...
    # Descobre as colunas de datas da aba de pagamentos.
    colunas_data = set()

    for registro in dados_pagamentos or []:
        for cabecalho in (registro or {}).keys():
            if _eh_coluna_data_pagamentos(cabecalho):
                colunas_data.add(limpar_texto(cabecalho))

    colunas_data = sorted(
        colunas_data,
        key=lambda valor: _data_pagamentos(valor) or datetime.max
    )
    ...
    for registro in dados_pagamentos or []:
        ...
        qtde = _inteiro_pagamentos(
            _chave_pagamentos(registro, "QTDE")
        )

        # Evita linha de totalização da própria planilha.
        if not sup and not sub and not com:
            continue

        chave = _chave_lideranca_pagamentos(
            sup,
            sub,
            com
        )
        ...
        total = _valor_monetario_pagamentos(
            _chave_pagamentos(registro, "TOTAL")
        )
        ...
        for cabecalho in colunas_data:
            valor = _valor_monetario_pagamentos(
                _chave_pagamentos(registro, cabecalho)
            )
            data_obj = _data_pagamentos(cabecalho)

            if not valor or not data_obj:
                continue
        ...
        # Se TOTAL tiver saldo que não esteja distribuído nas datas,
        # ele também entra em A PAGAR.
        saldo_sem_data = max(
            0.0,
            total - total_pago - a_pagar_datas
        )

        a_pagar = a_pagar_datas + saldo_sem_data
        ...
        if chave not in mapa:
            mapa[chave] = {
                ...
                "subsupervisor": sub,
                "comunidade": com,
                "lideranca": _nome_lideranca_pagamentos(
                    sup,
                    sub
                ),
                "qtde": 0,
        ...
        mapa[chave]["qtde"] += qtde
        mapa[chave]["pago"] += total_pago
        mapa[chave]["a_pagar"] += a_pagar
        mapa[chave]["total"] += total

    # --------------------------------------------------------
    # LIDERANÇAS CONTROLE -> ATUAL
    # --------------------------------------------------------
    for registro in dados_liderancas_controle or []:
        sup = limpar_texto(
            _chave_pagamentos(registro, "SUPERVISOR")
        )
        sub = limpar_texto(
        ...
        atual = _inteiro_pagamentos(
            _chave_pagamentos(registro, "ATUAL")
        )
        ...
        mapa[chave]["atual"] += atual

        if not mapa[chave]["supervisor"]:
            mapa[chave]["supervisor"] = sup
        if not mapa[chave]["subsupervisor"]:
            mapa[chave]["subsupervisor"] = sub
        if not mapa[chave]["comunidade"]:
            mapa[chave]["comunidade"] = com

        mapa[chave]["lideranca"] = (
            _nome_lideranca_pagamentos(
                mapa[chave]["supervisor"],
                mapa[chave]["subsupervisor"]
            )
        )

    registros = []

    for registro in mapa.values():
        if (
            fs
            and normalizar_filtro(
                registro.get("supervisor", "")
        ...
        # IMPORTANTE: mesma lógica do controle.
        registro["diferenca"] = (
            int(registro.get("qtde", 0))
            - int(registro.get("atual", 0))
        )

        # Compatibilidade com nomes usados no relatório antigo.
        registro["resta_pagar"] = registro["a_pagar"]
        registro["total_pago"] = registro["pago"]
    ...
    return {
        ...
        # Chaves de compatibilidade com a tela antiga.
        "total_resta_pagar": sum(
            r.get("a_pagar", 0.0)
            for r in registros
        ),
        "registros": registros,
...
def gerar_pdf_relatorio_pagamentos_resumidos(resultado_relatorio):
    ...
    documento = SimpleDocTemplate(
        ...
        title="Relatório de Pagamentos Resumidos"
    )

    estilos_base = getSampleStyleSheet()

    titulo = ParagraphStyle(
        "PagamentosResumidosTitulo",
        parent=estilos_base["Heading1"],
        fontName="Helvetica-Bold",
        fontSize=14,
        leading=16,
    ...
    centro = ParagraphStyle(
        "PagamentosResumidosCentro",
        parent=texto,
        alignment=TA_CENTER
    )

    cabecalho = ParagraphStyle(
        "PagamentosResumidosCabecalho",
        parent=centro,
        fontName="Helvetica-Bold",
        textColor=colors.white
    )
    ...
    dados = [[
        Paragraph("LIDERANÇA", cabecalho),
        Paragraph("QTDE", cabecalho),
        Paragraph("ATUAL", cabecalho),
        Paragraph("DIFERENÇA", cabecalho),
        Paragraph("TOTAL PAGO", cabecalho),
        Paragraph("A PAGAR", cabecalho),
        Paragraph("TOTAL", cabecalho),
    ]]

    registros = resultado_relatorio.get(
        "registros",
        []
    )
    ...
    for registro in registros:
        diferenca = int(
            registro.get("diferenca", 0)
        )

        dados.append([
            Paragraph(
                limpar_texto(
                    registro.get("lideranca", "")
                ) or "—",
                texto
            ),
            Paragraph(
                str(registro.get("qtde", 0)),
                centro
            ),
            Paragraph(
                str(registro.get("atual", 0)),
                centro
            ),
            Paragraph(
                str(diferenca),
    ...
    dados.append([
        Paragraph("<b>TOTAIS</b>", texto),
        Paragraph(
            f"<b>{resultado_relatorio.get('total_qtde', 0)}</b>",
            centro
        ),
        Paragraph(
            f"<b>{resultado_relatorio.get('total_atual', 0)}</b>",
            centro
        ),
        Paragraph(
            f"<b>{resultado_relatorio.get('total_diferenca', 0)}</b>",
            centro
        ),
        Paragraph(
            f"<b>{_formatar_moeda_pagamentos(resultado_relatorio.get('total_pago', 0))}</b>",
            centro
        ),
        Paragraph(
            f"<b>{_formatar_moeda_pagamentos(resultado_relatorio.get('total_a_pagar', 0))}</b>",
            centro
        ),
        Paragraph(
            f"<b>{_formatar_moeda_pagamentos(resultado_relatorio.get('total_geral', 0))}</b>",
            centro
        ),
    ])

    largura_util = pagina[0] - 1.10 * cm
    larguras_base = [
        8.2 * cm,
        2.0 * cm,
        2.0 * cm,
    ...
    fator = largura_util / sum(larguras_base)
    larguras = [
        largura * fator
        for largura in larguras_base
    ]
...
# ============================================================
# PAGAMENTOS RESUMIDOS - VERSÃO TABELA DINÂMICA
# A TABELA DINÂMICA é a fonte oficial de SUP/SUB/COM/ATUAL.
# ============================================================
...
def _chave_resumido_tabela_dinamica(supervisor, subsupervisor, comunidade):
    return (
        normalizar_filtro(supervisor),
        normalizar_filtro(subsupervisor),
        normalizar_filtro(comunidade),
    )
...
def gerar_relatorio_pagamentos_resumidos(
    ...
    # --------------------------------------------------------
    # PAGAMENTOS: indexa valores financeiros por SUP/SUB/COM.
    # --------------------------------------------------------
    colunas_data = set()
    for original in dados_pagamentos or []:
        for cabecalho in (original or {}).keys():
    ...
    for original in dados_pagamentos or []:
        ...
        chave = _chave_resumido_tabela_dinamica(sup, sub, com)
        item = pagamentos.setdefault(chave, {
            "qtde": 0,
            "pago": 0.0,
            "a_pagar": 0.0,
            "total": 0.0,
        ...
        item["qtde"] += qtde
        item["pago"] += pago
        item["a_pagar"] += a_pagar
        item["total"] += total

    # --------------------------------------------------------
    # TABELA DINÂMICA: BASE PRINCIPAL.
    # Nenhuma liderança dela é descartada por não ter pagamento.
    # --------------------------------------------------------
    dinamica = {}

    for original in dados_tabela_dinamica or []:
        sup = limpar_texto(_chave_pagamentos(original, "SUPERVISOR"))
        sub = limpar_texto(_chave_pagamentos(original, "SUBSUPERVISOR"))
        com = limpar_texto(_chave_pagamentos(original, "COMUNIDADE"))
        ...
        chave = _chave_resumido_tabela_dinamica(sup, sub, com)
        item = dinamica.setdefault(chave, {
            "supervisor": sup,
            "subsupervisor": sub,
            "comunidade": com,
            "atual": 0,
        })
        item["atual"] += _inteiro_pagamentos(
            _chave_pagamentos(original, "ATUAL")
        )

    registros = []

    for chave, base in dinamica.items():
        if fs and normalizar_filtro(base["supervisor"]) != fs:
            continue
        if fsub and normalizar_filtro(base["subsupervisor"]) != fsub:
            continue
        if fc and normalizar_filtro(base["comunidade"]) != fc:
            continue

        financeiro = pagamentos.get(chave, {
            "qtde": 0,
            "pago": 0.0,
            "a_pagar": 0.0,
            "total": 0.0,
        ...
        qtde = int(financeiro["qtde"])
        atual = int(base["atual"])

        registros.append({
            "supervisor": base["supervisor"],
            "subsupervisor": base["subsupervisor"],
            "comunidade": base["comunidade"],
            "qtde": qtde,
            "atual": atual,
            "diferenca": qtde - atual,
            "pago": financeiro["pago"],
            "a_pagar": financeiro["a_pagar"],
    ...
    registros.sort(
        key=lambda r: (
            ...
            normalizar_filtro(r["comunidade"]),
        )
    )
    ...
    return {
        ...
        "titulo": "Relatório de Pagamentos Resumidos",
        "total_liderancas": len(registros),
        "total_qtde": sum(r["qtde"] for r in registros),
        "total_atual": sum(r["atual"] for r in registros),
        "total_diferenca": sum(r["diferenca"] for r in registros),
        "total_pago": sum(r["pago"] for r in registros),
        "total_a_pagar": sum(r["a_pagar"] for r in registros),
...
def gerar_pdf_relatorio_pagamentos_resumidos(resultado_relatorio):
    ...
    AZUL = colors.HexColor("#175CD3")
    AZUL2 = colors.HexColor("#1570EF")
    VERDE = colors.HexColor("#039855")
    LARANJA = colors.HexColor("#DC6803")
    VERMELHO = colors.HexColor("#D92D20")
    ROXO = colors.HexColor("#6938EF")
    CINZA = colors.HexColor("#475467")
    FUNDO = colors.HexColor("#F2F4F7")
    ...
    elementos = [
        Paragraph("RELATÓRIO DE PAGAMENTOS RESUMIDOS", titulo),
        Paragraph("Conferência com a Tabela Dinâmica", pequeno),
    ]
    ...
    info = "Gerado em " + datetime.now(
        ZoneInfo("America/Maceio")
    ).strftime("%d/%m/%Y %H:%M")
    if partes:
        info += " &nbsp; | &nbsp; " + " &nbsp; | &nbsp; ".join(partes)

    elementos += [Paragraph(info, pequeno), Spacer(1, 0.12 * cm)]

    cards = [
        ("LIDERANÇAS", str(resultado_relatorio.get("total_liderancas", 0)), AZUL2),
        ("QTDE", str(resultado_relatorio.get("total_qtde", 0)), AZUL),
        ("ATUAL", str(resultado_relatorio.get("total_atual", 0)), VERDE),
        ("DIFERENÇA", str(resultado_relatorio.get("total_diferenca", 0)), LARANJA),
        ("TOTAL PAGO", _formatar_moeda_pagamentos(resultado_relatorio.get("total_pago", 0)), VERDE),
        ("A PAGAR", _formatar_moeda_pagamentos(resultado_relatorio.get("total_a_pagar", 0)), VERMELHO),
        ("TOTAL", _formatar_moeda_pagamentos(resultado_relatorio.get("total_geral", 0)), ROXO),
    ]

    card_data = [
        [Paragraph(f'<font color="{cor.hexval()}"><b>{rot}</b></font>', card_rotulo)
         for rot, valor, cor in cards],
        [Paragraph(valor, card_valor) for rot, valor, cor in cards],
    ]

    cards_tabela = Table(
        card_data,
        colWidths=[(pagina[0] - 1.10 * cm) / 7.0] * 7,
        rowHeights=[0.42 * cm, 0.62 * cm],
    )
    cards_tabela.setStyle(TableStyle([
    ...
    elementos += [cards_tabela, Spacer(1, 0.16 * cm)]
    ...
    dados = [[Paragraph(x, cab) for x in cabecalhos]]

    for r in resultado_relatorio.get("registros", []):
        dados.append([
            Paragraph(limpar_texto(r.get("supervisor", "")) or "—", celula),
            Paragraph(limpar_texto(r.get("subsupervisor", "")) or "—", celula),
            Paragraph(limpar_texto(r.get("comunidade", "")) or "—", celula),
            Paragraph(str(r.get("qtde", 0)), centro),
            Paragraph(str(r.get("atual", 0)), centro),
            Paragraph(str(r.get("diferenca", 0)), centro),
            Paragraph(_formatar_moeda_pagamentos(r.get("pago", 0)), centro),
            Paragraph(_formatar_moeda_pagamentos(r.get("a_pagar", 0)), centro),
            Paragraph(_formatar_moeda_pagamentos(r.get("total", 0)), centro),
        ])

    dados.append([
        Paragraph("<b>TOTAIS</b>", celula),
        "",
        "",
        Paragraph(f"<b>{resultado_relatorio.get('total_qtde', 0)}</b>", centro),
        Paragraph(f"<b>{resultado_relatorio.get('total_atual', 0)}</b>", centro),
        Paragraph(f"<b>{resultado_relatorio.get('total_diferenca', 0)}</b>", centro),
        Paragraph(f"<b>{_formatar_moeda_pagamentos(resultado_relatorio.get('total_pago', 0))}</b>", centro),
        Paragraph(f"<b>{_formatar_moeda_pagamentos(resultado_relatorio.get('total_a_pagar', 0))}</b>", centro),
        Paragraph(f"<b>{_formatar_moeda_pagamentos(resultado_relatorio.get('total_geral', 0))}</b>", centro),
    ])

    larguras = [4.2, 4.2, 3.6, 1.35, 1.35, 1.45, 2.45, 2.35, 2.45]
    util = pagina[0] - 1.10 * cm
    fator = util / (sum(larguras) * cm)

    tabela = Table(
        dados,
        colWidths=[x * cm * fator for x in larguras],
        repeatRows=1,
    )
    tabela.setStyle(TableStyle([
